...
class WorkflowTool(Tool):
    ...
    def description(self) -> str:
        return (
            ...
            "  `failed` — the plan has at least one failed step. You "
            "MAY send ONE follow-up `action='message'` with a "
            "substantively different angle to try (escalation that "
            "genuinely changes the approach, not rewording the same "
            "task); skip the retry if no clear alternative. If that "
            "retry also returns `failed` (or you skipped it), the run "
            "is over — no user approval needed. Call `preview` then "
            "`finalize` back-to-back, then deliver the outputs from "
            "finalize's response to the user via the `message` tool "
            "with a softened user-facing summary (use phrases like "
            "'couldn't fully complete' or 'ran into issues finishing' "
            "— 'failed' is internal language, not user-facing).\n"
