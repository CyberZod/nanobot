...
class AgentLoop:
    ...
    def _register_default_tools(self) -> None:
        ...
        # Custom workflow agency tool — needs the tool registry to look up the
        # `message` tool for a fixed start-of-workflow announcement, so it is
        # registered manually here (it sets _plugin_discoverable = False to opt
        # out of ToolLoader auto-discovery and avoid a double registration).
        # The Admissions demo tools are plain no-arg tools and auto-load.
        self.tools.register(WorkflowTool(tools=self.tools))
        registered.append("workflow")
    ...
    def _assemble_outbound(
        ...
        *,
        ...
        # MessageTool suppression. When follow-up messages were injected
        # mid-turn, a later natural-language reply may address those follow-ups
        # and should not be suppressed just because MessageTool was used earlier
        # in the turn. But if the turn falls back to the empty-final-response
        # placeholder, suppress it when the real user-visible output already came
        # from MessageTool. We also suppress when the final reply is a verbatim
        # repeat of what the tool already sent — common when the LLM restates the
        # message-tool caption as its final assistant text (would otherwise land
        # as a duplicate WhatsApp message).
